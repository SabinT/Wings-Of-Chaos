...
int main()
{
		...
		
	{
	...
	#ifdef TIXML_USE_STL
		{
			...
			// Round trip check: stream in, then stream back out to assert. The stream
	...
	printf ("\n** UTF-8 **\n");
	{
		...
		else
		{
			...
			FILE* assert = fopen( "utf8testverify.xml", "r" );
			if ( saved && assert )
			{
				while ( fgets( verifyBuf, 256, assert ) )
				{
			...
			XmlTest( "Legacy encoding: assert text element.", "r\x82sum\x82", text->Value() );
		}
	}		
