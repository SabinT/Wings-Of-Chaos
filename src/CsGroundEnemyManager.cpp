...
CsGroundEnemyManager::CsGroundEnemyManager(void)
{
}
...
void CsGroundEnemyManager::Load(Ogre::SceneNode *ogreScene, btBulletWorldImporter *bulletScene, const char *prefix) {
	...
	while (it.hasMoreElements()) {
			...
			(tempNode->getName().find("destroyed") == std::string::npos)) 
		{
			...
			if (tempBody) {
				CsGroundEnemy *tempObj = new CsGroundEnemy;
	...
	std::vector<CsGroundEnemy*>::iterator spawnObj = begin();
	while (spawnObj != end()) {
		(*spawnObj)->Spawn();
		spawnObj++;
	}
...
void CsGroundEnemyManager::Unload() {
	iterator it;
...
void CsGroundEnemyManager::UpdateSceneNodes() {
	iterator it;
	for (it = begin(); it != end(); it++) {
...
void CsGroundEnemyManager::Update() {
	iterator it;
	for (it = begin(); it != end(); it++) {
		if ((*it)->IsActive()) (*it)->Update();
	}
}

void CsGroundEnemyManager::Synchronize() {
	iterator it;
	for (it = begin(); it != end(); it++) {
		if ((*it)->IsActive()) (*it)->Synchronize();
	}
}